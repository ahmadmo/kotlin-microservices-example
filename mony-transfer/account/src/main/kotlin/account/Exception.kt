...
class ExceptionResponseStatusMapper(private val json: ObjectMapper) {
    ...
    fun map(error: Throwable): ResponseStatusException = when (error) {
        ...
        is BusyAccountException -> ResponseStatusException(HttpStatus.CONFLICT, "Busy Account")
        is NotEnoughBalanceException -> ResponseStatusException(HttpStatus.PRECONDITION_FAILED, "Not Enough Balance")
